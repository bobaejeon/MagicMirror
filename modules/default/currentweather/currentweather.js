...
Module.register("currentweather",{
	...
	//clothing depends on temperature
	addClothing: function(wrapper) {
		var small = document.createElement("div");
		small.className = "normal medium";
		
		var url = "./modules/default/currentweather/cloths/my-icons-collection/png/";
		var cloths = [["01","02","03"],["04","05","06"],["07","08","09"],["10","11","12"],["12","13","14"],["13","14","15"],["14","15","16"],["16","17","18"]];
		var index = null;

		if(this.temperature > 27) index = 0;
		else if(this.temperature > 22) index = 1;
		else if(this.temperature > 19) index = 2;
		else if(this.temperature > 16) index = 3;
		else if(this.temperature > 11) index = 4;
		else if(this.temperature > 8) index = 5;
		else if(this.temperature > 4) index = 6;
		else index = 7;

//졸라 커졌던 이유: medium에 width가 고정되지 않은 상태에서 %로 이미지 크기를 줘서 전체 화면의 %가 됨 => 고정크기 이미지로 변경
		for(var i in cloths[index]) {
			small.innerHTML += "<img class ='clothing' src='"+url+cloths[index][i]+".png'>"+"&nbsp;";
		}
		wrapper.appendChild(small);
	},
