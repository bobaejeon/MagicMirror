...
//var type = this.typeOfEvent(event.title) //뭐야 this안쓰면 안돼
eventDom.classList.add(this.typeOfEvent(event.title))
 ...
 //   var eventTitle = event.title
//var type = typeOfEvent(eventTitle)
//if(event.title.indexOf("기능") != -1)
//var tmp = this.temp()
//title.classList.add("exam")
//title.classList.add(type);
  ...
  typeOfEvent(title) {
  var type = "normal"
  if(title.indexOf("시험") != -1||title.indexOf("exam") != -1) type = "exam"
  else if(title.indexOf("과제") != -1||title.indexOf("test") != -1) type = "assignment"
  return type
}
}
